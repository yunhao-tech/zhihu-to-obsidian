...
一个 Chrome 浏览器插件，用于将中文知识网页转换为 Obsidian 兼容的 Markdown 格式。该插件在当前浏览器登录态下直接解析 DOM 与调用 API，重点解决中文平台内容归档时常见的格式丢失问题：公式、图片、代码块、引用卡片、元数据和专栏结构。

> 这不是通用 Web Clipper，而是面向中文长文、技术文章、专栏和本地知识库/RAG 语料归档的高保真导出工具。

---

## Paid Beta

本项目正在验证一个明确的小目标：把中文知识网页归档做成可持续维护的本地知识库工具。

- **Pro Beta**：$99/年，包含多平台导出、知乎专栏批量导出、本地图片打包、Tailscale/NAS 图片路径、导出模板和兼容性更新。
- **Concierge Archive**：$99/次，协助将一个你有权限访问的中文专栏/文章集合整理成 Obsidian-ready Markdown vault。

如果你是重度 Obsidian / PKM / RAG / 中文长文归档用户，可以先通过 issue 或邮件联系参与 beta。

本插件不会绕过平台权限或付费墙；它只在你自己的浏览器登录态中，导出你本来就可以访问的页面内容。
...
> 这些平台第一版采用“平台选择器 + 通用 HTML 转 Markdown”策略，优先解决单篇文章归档。知乎保留专栏批量导出和平台特化处理。
...
- **中文平台文章导出**：
  支持微信公众号、掘金、CSDN、博客园、少数派、简书等中文内容平台的单篇文章导出，适合沉淀技术资料、课程笔记和 RAG 语料。
- **专栏批量导出**：
...
### 导出
1. 在网页端登录并打开期望导出的 **知乎文章页/专栏页**，或其他受支持的中文文章页面。
